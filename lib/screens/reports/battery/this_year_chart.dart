...
class _ThisYearChartState extends State<ThisYearChart> {
  ...
  void fetchDataFromApi() async {
   ...
   var token = storage.getItem('token');
    String url = '$baseur/v1/reports/battery_statistics/year';
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<DataPoint> dataPoints = List<DataPoint>.from(data.map((item) {
          return DataPoint(item['month'], item['swaps']);
        }));
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: _dataPoints.isNotEmpty
          ? SfCartesianChart(
            title: ChartTitle(text: 'This Year BAT Swaps', textStyle: GoogleFonts.teko()),
            legend: const Legend(
            ...
            tooltipBehavior:  _tooltipBehavior,
              ...
              primaryXAxis: NumericAxis(
                ...
                title: AxisTitle(text: 'Months of the year', textStyle: const TextStyle(color:Colors.green, fontSize: 14))),
