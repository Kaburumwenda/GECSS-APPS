...
class _ThisYearlyChartState extends State<ThisYearlyChart> {
  ...
  void fetchDataFromApi() async {
   ...
   var token = storage.getItem('token');
    String url = '$baseur/v1/reports/battery_statistics/yearly';
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<DataPoint> dataPoints = List<DataPoint>.from(data.map((item) {
          return DataPoint(item['year'], item['swaps']);
        }));
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: _dataPoints.isNotEmpty
          ? SfCartesianChart(
            title: ChartTitle(text: 'Yearly BAT Swaps', textStyle: GoogleFonts.teko()),
            legend: const Legend(
