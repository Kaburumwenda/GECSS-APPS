...
class ThisYearAreaChart extends StatefulWidget {
  const ThisYearAreaChart({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ThisYearAreaChartState createState() => _ThisYearAreaChartState();
}

class _ThisYearAreaChartState extends State<ThisYearAreaChart> {
  ...
  void fetchDataFromApi() async {
    ...
    String url = '$baseur/v1/reports/mpesa_statistics/year';
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<DataPoint> dataPoints = List<DataPoint>.from(data.map((item) {
          return DataPoint(item['month'], item['total']);
        }));
        setState(() {
          ...
          ? SfCartesianChart(
            ...
            tooltipBehavior:  _tooltipBehavior,
              ...
              primaryYAxis: NumericAxis(
                ...
                labelFormat: '{value}', 
                title: AxisTitle(text: 'Amount Earned in - KES', textStyle: const TextStyle(
...
class DataPoint {
  ...
  DataPoint(this.month, this.total);
}
